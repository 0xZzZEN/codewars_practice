...
void spin_words(const char *sentence, char *result) 
{
    ...
    unsigned int current_letter_position_in_sentence = 0, word_count = 0, start_position_of_the_word = 0;
    ...
    while (1)
    {
        ...
        if ((sentence[current_letter_position_in_sentence] == ' ') || (sentence[current_letter_position_in_sentence] == '.') || (sentence[current_letter_position_in_sentence] == ',') || (sentence[current_letter_position_in_sentence] == '\0'))
        {
            ...
            unsigned int end_position_of_the_word = current_letter_position_in_sentence - 1; // minus one means without SPACE or NUL char
            unsigned int num_of_letters_of_current_word = end_position_of_the_word - start_position_of_the_word; // count starts from zero

            word_count += 1; // word counter

            /*if (num_of_letters_of_current_word >= 5)
            {
            char *pointer_to_the_first_letter = &sentence[start_position_of_the_word];
            char *pointer_to_the_end_letter = &sentence[end_position_of_the_word];
            reordered_word[i] = sentence[end_position_of_the_word] 
            }
            else 
            {
            reordered_word[i] 
            ...
            for (unsigned short i = 0; i <= num_of_letters_of_current_word; i++)
            {
                if (num_of_letters_of_current_word >= 5)
                {
                // if word has 5 or more letters, we should reverse the order and store it in a new sentence
                result[start_position_of_the_word + i] = sentence[end_position_of_the_word - i];
                }
                else
                {
                // else we should use the original word from sentence and only place the original order into a new sentence
                result[start_position_of_the_word + i] = sentence[start_position_of_the_word + i];
                }
            }
            ...
            {
                // store NUL char at the end of sentence and break
